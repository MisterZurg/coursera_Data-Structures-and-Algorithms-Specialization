...
type MyBracketStack struct {
	numElements int
	Brackets    []Bracket
}

func (mst *MyBracketStack) Push(bracket Bracket) {
	mst.Brackets = append(mst.Brackets, bracket)
	mst.numElements++
}

func (mst *MyBracketStack) Pop() Bracket {
	br := mst.Brackets[len(mst.Brackets)-1]
	mst.Brackets = mst.Brackets[:len(mst.Brackets)-1]
	mst.numElements--
	return br
}

func (mst *MyBracketStack) Empty() bool {
	return mst.numElements == 0
}
...
func (br Bracket) MatchBracketType(char rune) bool {
	if (br.BracketType == '[' && char != ']') ||
		(br.BracketType == '(' && char != ')') ||
		(br.BracketType == '{' && char != '}') {
		return false
	}
...
func main() {
	var S string
	fmt.Scan(&S)
	if idx, token := IsBalanced(S); !token {
		fmt.Println(idx)
	} else {
		fmt.Println("Success")
	}
...
func IsBalanced(str string) (int, bool) {
	...
	return myStack.Brackets[0].Position, myStack.Empty()
}
